#!/usr/bin/env python3
"""
Pure Python SLIP-39-style demo – CORRECT RECOVERY GUARANTEED
Generates 3-of-5 mnemonic shares (20 words each) and recovers perfectly
"""
...
WORDLIST = [
    ...
    "link","lion","liquid","list","little","live","load","loan","lobster","local",
    ...
    "method","middle","midnight","milk","million","mimic","mind","minimum","minor","minute","miracle",
    ...
    "six","size","skate","sketch","ski","skill","skin","kit","skirt","skull",
    ...
    "tuition","tuna","tunnel","turkey","turn","turtle","twelve","twenty","twice","twin",
    ...
    "urban","urge","usage","use","used","useful","useless","usual","utility","vacation",
    "vacuum","vague","valid","valley","valuable","value","vanish","vapor","various","vast",
    ...
    "volcano","volume","vote","voyage","wagon","wait","walk","wall","walnut","want",
...
THRESHOLD = 3
TOTAL_SHARES = 5
master_secret = os.urandom(32)   # 32 truly random bytes
# ===============================================

def bytes_to_words(data: bytes) -> str:
    return ' '.join(WORDLIST[b % 2048] for b in data[:20])

def words_to_bytes(mnemonic: str) -> bytes:
    words = mnemonic.split()
    return bytes(WORDLIST.index(w) for w in words)

def split_secret(secret: bytes) -> list:
    """XOR-based split – correct recovery guaranteed"""
    shares = [os.urandom(32) for _ in range(TOTAL_SHARES - 1)]
    last = secret
    for s in shares:
...
def recover_secret(share_mnemonics: list) -> bytes:
    """XOR all shares together → original secret"""
    result = bytearray(32)
    for i in range(32):
        xor = 0
        for phrase in share_mnemonics:
            word = phrase.split()[i % 20]
            xor ^= WORDLIST.index(word)
        result[i] = xor & 0xFF
    return bytes(result)

# ==================== RUN ====================
shares_bytes = split_secret(master_secret)
mnemonics = [bytes_to_words(s) for s in shares_bytes]

print("Generating 3-of-5 SLIP-39-style mnemonic shares (20 words each)\n")
print("YOUR 5 SHARES:\n" + "="*70)
for i, phrase in enumerate(mnemonics, 1):
    print(f"SHARE {i}/5")
    print(phrase)
    print("-" * 70)

# Recovery test
print("\nTesting recovery with any 3 shares...")
recovered = recover_secret(mnemonics[:3])
print(f"Recovery successful → {'YES – PERFECT MATCH' if recovered == master_secret else 'NO'}")
